...
**Текущий релиз: [v2.0.0](CHANGELOG.md)** — крупное обновление рабочего процесса,
локального backend и интерфейса.
...
- Перегенерация caption с необязательной подсказкой о деталях, которые модель пропустила или описала неверно
...
- Единая рабочая область: dataset → scan → Preview → Run → Results
- Run Center с историей, параметрами, итогами и provenance
- Фоновый Preview выбранного pipeline без записи в dataset
...
1. В разделе **«Работа»** выберите dataset и просканируйте его.
2. Проверьте readiness и выполните Preview: результат не записывается на диск.
3. Откройте «Настройка и запуск», выберите режим/профиль и запустите Run.
4. Следите за Run в разделе **«Запуски»**, затем проверьте **«Результаты»**.

Автоматическая конфигурация используется по умолчанию. Ручные параметры backend и
модели находятся в закрытом блоке **Advanced / Expert**, а effective configuration и
экспорт сведений — в разделе **«Диагностика»**.
...
<details>
<summary>Managed llama.cpp (рекомендуется)</summary>

При первом запуске Tag Manager сам скачивает последнюю официальную Windows CUDA 12.4
сборку llama.cpp вместе с CUDA runtime, проверяет SHA256 и устанавливает её рядом с
приложением. Командная строка и ручной поиск `llama-server.exe` не нужны.

Пользователю остаётся скачать мультимодальную GGUF-модель и её `mmproj-*.gguf`, указать
папки в сайдбаре и нажать **Start llama.cpp**. Если используется совместимая portable
установка text-generation-webui, папки `user_data/models` и `user_data/mmproj`
сканируются напрямую. Внешний OpenAI-compatible API по-прежнему доступен как fallback.

Режим **Авто** читает GGUF metadata и подбирает параметры под доступные RAM/VRAM.
В режиме **Вручную** доступны context, GPU layers, KV cache K/V, Flash Attention,
batch/ubatch, threads, load mode, reasoning/output budgets и другие runtime-параметры.
Наборы настроек можно сохранять как отдельные backend-профили для каждой модели.
